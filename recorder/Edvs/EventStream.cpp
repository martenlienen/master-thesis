...
public:
  ...
  void open(const std::string &uri) { h = edvs_open(uri.c_str()); }
  ...
  bool is_open() const { return h; }

  bool is_master() const { return edvs_get_master_slave_mode(h) == 1; }

  bool is_slave() const { return edvs_get_master_slave_mode(h) != 1; }
  ...
  bool is_live() const { return edvs_is_live(h) == 1; }
  ...
  uint64_t last_timestamp() const { return last_time_; }
...
private:
  std::vector<edvs_event_t> pop_events() {
    ...
    std::vector<edvs_event_t> tmp =
        std::move(events_); // TODO is this move correct?
    events_ = {};
    return tmp;
  }
...
public:
  ...
  unsigned num_streams() const { return streams_.size(); }
  ...
  std::vector<edvs_event_t> read() {
    ...
    std::sort(
        ...
        [](const edvs_event_t &a, const edvs_event_t &b) { return a.t < b.t; });
    // find location of common time in our buffer
    auto it = std::upper_bound(
        events_.begin(), events_.end(), common_time,
...
std::shared_ptr<IEventStream>
OpenEventStream(const std::initializer_list<std::string> &uris) {
  return std::make_shared<MultiEventStream>(uris);
}

std::shared_ptr<IEventStream>
OpenEventStream(const std::vector<std::string> &uris) {
  return std::make_shared<MultiEventStream>(uris);
}
}
