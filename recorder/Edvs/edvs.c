...
#include <termio.h>
#define MEDVS 12000000 // right baudrate for mini-edvs

int edvs_serial_open(const char *path, int baudrate) {
  int baudrate_enum;
  switch (baudrate) {
  case 2000000:
    baudrate_enum = B2000000;
    break;
  case 4000000:
    baudrate_enum = B4000000;
    break;
  case 12000000:
    break;
  default:
  ...
  if (baudrate == MEDVS) {
    ...
    ioctl(port, TIOCGSERIAL, &ser); // get the current port options
    printf("Baud_base = %d\n",
    ...
    ser.custom_divisor =
        ser.baud_base /
        baudrate; // compute the divisor to apply to base baudrate
    printf("custom_divisor = %d\n", ser.custom_divisor);
    printf("closest baud rate = %d\n", ser.baud_base / ser.custom_divisor);
    int error = ioctl(port, TIOCSSERIAL, &ser); // set the options

    flags = B38400;               // default flags
    tcgetattr(port, &options);    // get current options from port
    cfsetispeed(&options, flags); // set input speed
    cfsetospeed(&options, flags); // set output speed
    options.c_cflag =
        (options.c_cflag & ~CSIZE) | CS8; // transmission size to 8bit
    options.c_cflag |= (CLOCAL | CREAD);  // enable receiver and set local mode
    options.c_cflag |= CRTSCTS;           // use hardware handshaking (RTS/CTS)
    options.c_iflag = IGNBRK; // clear input options: Send a SIGINT when a break
    ...
    tcsetattr(port, TCSANOW, &options); // set new options to port
    tcflush(port, TCIFLUSH);            // flush input buffer
    return port;
  }
  ...
  settings.c_lflag = 0;
  settings.c_cc[VMIN] =
      1; // minimum number of characters to receive before satisfying the read.
  settings.c_cc[VTIME] =
      5; // time between characters before satisfying the read.
...
uint64_t timestamp_limit(int mode) {
  ...
  case 1:
    return (1ull << 16); // 16 bit
  case 2:
    return (1ull << 24); // 24 bit
  case 3:
    return (1ull << 32); // 32 bit
  }
}
...
edvs_device_streaming_t *edvs_device_streaming_open(edvs_device_t *dh,
                                                    int device_tsm,
                                                    int host_tsm,
                                                    int master_slave_mode) {
  edvs_device_streaming_t *s =
      (edvs_device_streaming_t *)malloc(sizeof(edvs_device_streaming_t));
  if (s == 0) {
    return 0;
  }
  s->device = dh;
...
/** Unwraps timestamps the easy way (works good for 3 byte ts, be careful with 2
 * byte ts) */
void compute_timestamps_incremental(edvs_event_t *begin, size_t n,
                                    uint64_t last_device, uint64_t last_host,
                                    uint64_t wrap) {
...
uint64_t sum_dt(edvs_event_t *begin, edvs_event_t *end, uint64_t last_device,
                uint64_t wrap) {
...
void compute_timestamps_systime(edvs_event_t *begin, size_t n,
                                uint64_t last_device, uint64_t last_host,
                                uint64_t wrap, uint64_t systime) {
  edvs_event_t *end = begin + n;
  ...
  // if dt_D < dt_H: this might have a natural cause, so we do not know if we
  // need to do something
  // if dt_D > dt_H: we have a problem as timestamps would not be ordered. to
  // solve this we scale timestamps
  uint64_t rem = 0;
  //	printf("%"PRIu64"\t%"PRIu64"\n", (end - 1)->t, systime);
  uint64_t curr_host = systime;
  uint64_t next_set_host = systime;
  for (edvs_event_t *events = end - 2; events >= begin;
       --events) { // iterate backwards (skip last)
    // current device time (wrapped)
    uint64_t t = events->t;
    // current delta time
    ...
    curr_host -= dt_host;
    (events + 1)->t = next_set_host; // delay set to not corrupt device
                                     // timesteps needed for computation
...
ssize_t edvs_device_streaming_read(edvs_device_streaming_t *s,
                                   edvs_event_t *events, size_t n,
                                   edvs_special_t *special, size_t *ns) {
  // realtime timer
  ...
  const unsigned int cNumBytesAhead = (cNumBytesPerEvent > cNumBytesPerSpecial)
                                          ? cNumBytesPerEvent
                                          : cNumBytesPerSpecial;
  // read bytes
  ...
  size_t num_read = (num_bytes_buffer < num_bytes_events ? num_bytes_buffer
                                                         : num_bytes_events);
  ssize_t bytes_read =
      edvs_device_read(s->device, buffer + s->offset, num_read);
#ifdef EDVS_LOG_ULTRA
...
//			printf("s: len=%ld\n", special_data_len);
    ...
    uint64_t timestamp;
    if (timestamp_mode == 1) {
      timestamp = ((uint64_t)(buffer[i]) << 8) | (uint64_t)(buffer[i + 1]);
    } else if (timestamp_mode == 2) {
      timestamp = ((uint64_t)(buffer[i]) << 16) |
                  ((uint64_t)(buffer[i + 1]) << 8) | (uint64_t)(buffer[i + 2]);
// printf("%d %d %d %d %d\n", a, b, buffer[i+0], buffer[i+1], buffer[i+2]);
      ...
      timestamp = ((uint64_t)(buffer[i]) << 24) |
    ...
    // 			// FIXME this does not assure that timestamps are
    // increasing!!!
    // 			uint64_t dt;
    // 			if(timestamp < s->last_timestamp) {
    // 				// we have a wrap
    // 				dt = cTimestampLimit + timestamp -
    // s->last_timestamp;
    // 			}
    // 			else {
    ...
    // 			// // 	// FIXME this is problematic with multiple event
    // streams
    // 			// // 	//       as they will have different offsets
    // 			// // 	s->last_timestamp = system_clock_time;
    // 			// 	s->last_timestamp = 0; // use system clock time
    // zero
    // point
    // 			// }
    ...
    // 			if(timestamp_mode != 0) {
    // 				if(s->current_time < 8) { // ignore timestamps of first
    // 8
    // events
    // 					s->current_time ++;
    // 				}
    ...
    // 					if(s->last_timestamp != cTimestampLimit)
    // {
    // 						// FIXME possible errors for 16 bit timestamps if no event for
    // more
    // than 65 ms
    // 						// FIXME possible errors for 24/32 bit timestamps if timestamp
    // is
    // wrong
    // 						if(timestamp >=
    // s->last_timestamp)
    ...
    // *
    // timestamp;
    // 							s->current_time += timestamp + (cTimestampLimit
    // -
    // s->last_timestamp);
    // 						}
    // 					}
...
// read special data
  ...
  if (num_events > 0) {
    ...
    if (s->host_timestamp_mode == 1) {
      compute_timestamps_incremental(events, num_events, last_device, last_host,
                                     cTimestampLimit);
    }
    if (s->host_timestamp_mode == 2) {
      compute_timestamps_systime(events, num_events, last_device, last_host,
                                 cTimestampLimit,
                                 system_clock_time - s->systime_offset);
    }
...
int edvs_device_streaming_write(edvs_device_streaming_t *s, const char *cmd,
                                size_t n) {
...
edvs_file_streaming_t *edvs_file_streaming_open(const char *filename,
                                                uint64_t dt, float ts) {
  edvs_file_streaming_t *s =
      (edvs_file_streaming_t *)malloc(sizeof(edvs_file_streaming_t));
  if (s == 0) {
    return 0;
  }
...
ssize_t edvs_file_streaming_read(edvs_file_streaming_t *s, edvs_event_t *events,
                                 size_t events_max) {
  if (s->is_eof) {
    return 0;
  }
  ...
  if (s->dt == 0) {
    ...
    s->current_event_time =
        s->start_event_time + (uint64_t)(s->timescale * (float)(nt));
  } else {
...
int parse_uri_device(const char *curi, char **name, int *baudrate, int *dtsm,
                     int *htsm, int *msmode) {
  // Example URI:
...
edvs_stream_handle edvs_open(const char *uri) {
  ...
  if (uri_type == 1) {
    ...
    // open device
    printf("Opening network socket: ip=%s, port=%d using device_tsm=%d, "
           "host_tsm=%d, master/slave=%d\n",
           ip, port, dtsm, htsm, msmode);
    int dev = edvs_net_open(ip, port);
    if (dev < 0) {
      free(ip);
      printf("edvs_open: URI seems to point to a network socket, but "
             "connection failed\n");
      return 0;
    }
    free(ip);
    ...
    dh->handle = dev;
    edvs_device_streaming_t *ds =
        edvs_device_streaming_open(dh, dtsm, htsm, msmode);
    struct edvs_stream_t *s =
        (struct edvs_stream_t *)malloc(sizeof(struct edvs_stream_t));
    ...
    // open device
    printf("Opening serial port: port=%s using baudrate=%d, device_tsm=%d, "
           "host_tsm=%d, master/slave=%d\n",
           port, baudrate, dtsm, htsm, msmode);
    int dev = edvs_serial_open(port, baudrate);
    if (dev < 0) {
      free(port);
      printf("edvs_open: URI seems to point to a serial port, but connection "
             "failed\n");
      return 0;
    }
    free(port);
...
ssize_t edvs_read_ext(edvs_stream_handle s, edvs_event_t *events, size_t n,
                      edvs_special_t *special, size_t *ns) {
  if (s->type == EDVS_DEVICE_STREAM) {
    edvs_device_streaming_t *ds = (edvs_device_streaming_t *)s->handle;
