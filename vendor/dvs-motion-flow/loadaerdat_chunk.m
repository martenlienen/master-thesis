...
switch version
    case 1
        allAddr=uint16(fread(f,ev_end-ev_start,'uint16',4,'b')); % addr are each 2 bytes (uint16) separated by 4 byte timestamps
        fseek(f,bof+numBytesPerEvent*ev_start+2,'bof'); % timestamps start 2 after bof
        allTs=uint32(fread(f,ev_end-ev_start,'uint32',2,'b')); % ts are 4 bytes (uint32) skipping 2 bytes after each
    case 2
        allAddr=uint32(fread(f,ev_end-ev_start,'uint32',4,'b')); % addr are each 4 bytes (uint32) separated by 4 byte timestamps
        fseek(f,bof+numBytesPerEvent*ev_start+4,'bof'); % timestamps start 4 after bof
        allTs=uint32(fread(f,ev_end-ev_start,'uint32',4,'b')); % ts are 4 bytes (uint32) skipping 4 bytes after each
end
